...
class _WikiTableState extends State<WikiTable> {
  ...
  init() async {
    try {
      final resp = await http.Client().get(Uri.parse(widget.url));
  ...
  void exportToCsv() {
    if (filmDetailsList.isEmpty) {
      ...
      try {
        ...
        rows.add(['Sr No.', 'Movie', ...filmDetailsList[0].details.keys]);

        for (var index = 0; index < filmDetailsList.length; index++) {
          var film = filmDetailsList[index];
          var detailsValues = film.details.values
              .map((value) => value.isNotEmpty ? value : 'N/A')
              .toList();

          List<dynamic> row = [index + 1, film.title, ...detailsValues];
          rows.add(row);
        }
